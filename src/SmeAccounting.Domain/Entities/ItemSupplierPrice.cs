...
public class ItemSupplierPrice : BaseEntity
{
        ...
        DateOnly? effectiveTo = null)
    {
        ...
        if (supplierId <= 0)
            throw new DomainException("SupplierId must be greater than zero.");
        if (itemId <= 0)
        ...
        AddDomainEvent(new ItemSupplierPriceCreated(Id, companyId, DateTimeOffset.UtcNow));
    }
