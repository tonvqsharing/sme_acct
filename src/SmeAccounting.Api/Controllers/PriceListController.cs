...
public class PriceListController : Controller
{
    ...
    {
        var priceLists = await _mediator.Send(new GetPriceListsByCompanyQuery(companyId), ct);
    ...
    public async Task<IActionResult> Create(long companyId, string code, string name, string? description, CancellationToken ct)
    {
        try
        {
            var command = new CreatePriceListCommand(companyId, code, name, description);
    ...
    {
        await _mediator.Send(new DeactivatePriceListCommand(id), ct);
        return RedirectToAction(nameof(Index), new { companyId });
    }
}