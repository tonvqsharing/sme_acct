...
# Chart of Accounts Module (Lego Brick)
_Brick: `src/bricks/coa/`. Pure Python domain. Flask built-in RBAC. SQLite3 default DB._
...
src/bricks/
  coa/                         ← 🧱 NEW brick
    contract.py                ← 🔌 Public interface (AccountCode, AccountCategory, primitive IDs only)
    domain.py                  ← 🎯 Account, AccountStatus entities (pure Python)
    services.py                ← ⚙️ CoaService
    storage.py                 ← 💾 SQLAlchemy models + repository adapters
    web_adapter.py             ← 🌐 Flask blueprint + REST endpoints (coa_bp)
```
...
### 1.1 `src/bricks/coa/domain.py` — Account Aggregate Root
...
### 1.2 `src/bricks/coa/domain.py` — Supporting Entities
...
### 1.3 Domain Exceptions (in `src/bricks/coa/domain.py`)
...
## 3. Storage Adapters (bricks/coa/storage.py)

### 3.1 `src/bricks/coa/storage.py` — Repository Adapters
...
### 3.2 `src/bricks/coa/storage.py` — Add model imports

Add `AccountModel`, `AccountCategoryModel`, `AccountTagModel`, and `account_tag_xref` to the storage file. Ensure `lazy="selectin"` on relationships (per codebase convention). Add `UniqueConstraint("company_id", "code")` on `AccountModel`.
...
## 4. Services Layer (bricks/coa/services.py)

### 4.1 `src/bricks/coa/services.py`
...
- `@login_required` + `current_user.role` checks on API routes (like `currencies_bp.py` pattern)
...
def list_accounts():
    if current_user.role not in READ_ROLES:
        abort(403, description="RBAC denied: read role required")
...
@login_required
# AUTO_SEED_ROLES check: no AUDITOR; write operation
def create_account():
    try:
...
@login_required
# FY_ADMIN_ROLES check: CHIEF_ACCOUNTANT/ADMIN/DIRECTOR only
...
| **Security** | `@login_required + current_user.role` on all routes; AUDITOR read-only (excluded from AUTO_SEED_ROLES); actor UUID on all mutations (D11); no plain-text secrets; audit checksums SHA-256 immutable |
...
- Existing module patterns: `currencies_bp.py` (service per-request, test-engine hook); `fiscal_year_bp.py` (9 routes, `@login_required + current_user.role`); `system_settings_bp.py` (broken, adapter missing — out of scope)
...
1. Create `src/bricks/coa/domain.py` + domain exceptions
2. Create `src/bricks/coa/storage.py` with repository adapters + model definitions
3. Create manual migration `migrations/versions/a1b2c3d4e5f6_add_coa_tables.py`
4. Create `src/bricks/coa/services.py`
5. Create `src/bricks/coa/web_adapter.py` + serializers
6. Create `src/bricks/coa/contract.py` (public interface)
7. Create unit + integration test files
8. Run full suite regression; ensure 262 pass baseline preserved