...
- [opening-balance-pr-link, G2] List-backed fakes must simulate EF identity (counter-based Id assignment in AddAsync) or domain Id>0 guards throw at runtime: FakeJournalEntryRepository needed `entry.Id = _nextId++` or PR ctor V2 guard (journalEntryId > 0) failed every handler fact with DomainException; prove the runtime RED first (production changed, enabler not yet applied), then apply enabler → green.
- [opening-balance-pr-link, G3] RED-reconstruction canon refinement (MEMORY:255): revert ONLY the production change (`git checkout <commit>~1 -- <production file>`), keep tests at GREEN — reverting the WHOLE commit diff (`git apply -R` of all files) restores the self-consistent pre-change baseline which compiles, so a compile RED that was an INTERMEDIATE state (tests updated, production not yet) does NOT reproduce and the reconstruction proves nothing; "changed file(s)" in the canon means the production file, not every file in the commit.

### Business Partners — CustomerGroup/SupplierGroup/SupplierItem (Sep 2026)
- [business-partners, G1] 3-entity slice pattern (51 CREATE + 10 EDIT, all VERIFIED_PASS/CLEAN): master-data group entity (CompanyId, Code 20, Name 200, IsActive, Description 500, unique (CompanyId,Code), Company FK Restrict, xmin, Ignore-only DbContext + Set<T>() repo + flat CQRS + thin controller) + link entity (UserRole-shaped, CompanyId FIRST in ctor/command — deviates from UserRole userId-first, MEMORY:263 wins) + nullable-FK append-on-existing-entity (property before IsActive, ctor param LAST with `HasValue && <= 0` guard, PostingConfiguration.cs:32 pattern — existing callers compile unchanged).
- [business-partners, G1] Partial-unique-index link-entity lesson: plain unique triple on a soft-delete link entity = dead-end on deactivate→re-add (DbUpdateException, no Reactivate/no dup pre-check); fix = `HasIndex((CompanyId, SupplierId, ItemId)).IsUnique().HasFilter("\"is_active\"")` — uniqueness among ACTIVE rows only, inactive row stays for audit. First HasFilter in codebase; EF Core 10 + Npgsql render `filter: "is_active"` in migration. Rejected: include IsActive in index columns (allows exactly one re-add cycle, then collides).
- [business-partners, G1] Nullable-FK-append-on-existing-entity canon: `AddColumn nullable:true` with NO defaultValue ⇒ zero backfill risk (MEMORY:248 trap avoided); explicit non-unique `HasIndex` for FK lookup is optional (EF convention creates it anyway — proven by IX_suppliers_payment_term_id).
- [business-partners, G1] Parallel-task shared-file race on DbContext/DI: re-read before edit; placement rule = insert Ignore/AddScoped directly after the related entity's line (Supplier events/repos adjacent). MSBuild zombie hang: `pkill -f "MSBuild.dll"` then retry. Stale LSP diagnostics ≠ real errors — build is arbiter.
- [business-partners, G1] Migration carry-forward (T5): ONE consolidated migration = 3 CreateTable (customer_groups, supplier_groups, supplier_items) + 2 nullable AddColumn NO defaultValue (customer_group_id on customers, supplier_group_id on suppliers) + partial unique filter "is_active" on supplier_items triple + all FKs Restrict; check-only, never `database update` without approval.

### Business Partners — Test Slice (Sep 2026)
- [business-partners, G2] List-backed-fake + Id=5-before-AddAsync canon for Deactivate-handler tests (extends MEMORY:258): plain Add (NO `_nextId` counter) is correct when the entity ctor has no own-Id>0 guard (transient Id=0, `result.Id == stored.Id == 0` passes); Deactivate-happy facts MUST set `entity.Id = 5` (BaseEntity.Id public setter) BEFORE AddAsync or GetByIdAsync(5) returns null — and assert SaveCalledCount==1 because Deactivate handlers DO save (SaveChangesAsync :21 each). Explicit `using SmeAccounting.Application.Handlers;` required (internal handlers, CS0246); check each fake against its actual port file for member completeness (FakeSupplierItemRepository = 3 members, no GetByCodeAsync) or CS0535.
- [business-partners, G2] T5 migration carry-forward: ONE consolidated migration = 3 CreateTable + 2 nullable AddColumn NO defaultValue + PARTIAL unique filter "is_active" on supplier_items (company_id, supplier_id, item_id) triple + all FKs Restrict; check-only, never `database update`; count 17→18.
- [business-partners, G2] BankTests baseline for future loops: 115/115 (72 pre-partners + 43 new: CustomerGroup 14 + SupplierGroup 14 + SupplierItem 15) — use as the new unchanged-baseline gate for any future src/ slice.

### Business Partners — Migration + Loop Complete (Sep 2026)
- [business-partners, G3] psql live-DB absence check = new evidence canon item for check-only migration loops: `select tablename from pg_tables where schemaname='public' and tablename in (...)` → 0 rows proves `database update` never ran — pairs with scaffold-file checks (migration count, R1-R7, snapshot delta) as the closed-loop evidence set.
- [business-partners, G3] Partial-unique HasFilter rendering CONFIRMED: EF Core 10 + Npgsql render `filter: "\"is_active\""` in migration CreateIndex exactly as configured (`HasFilter("\"is_active\"")`) — first HasFilter in codebase, renders verbatim at :131-136; snapshot carries it too.
- [business-partners, G3] Migration baseline: `AddPartnerGroupsAndSupplierItems` (20260924003244) = 18 migrations total / 37 .cs — new baseline for any future migration loop; **APPLIED to live dev DB sme_acct_dev 2026-09-24 after explicit user approval** (verified: 3 tables present, nullable group_id columns, partial unique index `WHERE is_active`, 11 partner FKs all Restrict, history row recorded).
- [business-partners, G3] Module final state: CustomerGroup/SupplierGroup/SupplierItem full slices (5 layers each, 51 CREATE + 10 EDIT), BankTests 115/115, build 0/0, arch 22/22 — loop complete.

### Journal Entry Handlers — Cross-Loop Reference (Sep 2026)
- [journal-entry-handlers, G1] `[Required]` on non-nullable `long` is a no-op (long never null) — real guard is FluentValidation `GreaterThan(0)`; empty number input binds 0, validator rejects (PeriodId pattern).
- [journal-entry-handlers, G1] CompanyId is FIRST param in 30/32 Create commands (exceptions: CreateVoucherTypeCommand, CreateAccountCommand 4th) — new Create commands put CompanyId first; PaymentMethod/Customer canonical.
- [journal-entry-handlers, G1] ViewModel CompanyId without a view input silently posts 0 — ChartOfAccounts/Create.cshtml broken (VM has CompanyId, view omits input); JournalEntry/Create.cshtml is first view rendering visible CompanyId number input.
- [journal-entry-handlers, G1] Command-shape change surface = 5 files: command record, validator, ViewModel, view, controller; grep call sites first — CreateJournalEntryCommand had exactly 2 (validator + controller), no tests/handler.
- [journal-entry-handlers, G1] Style split: FluentValidation messages English ("Company ID is required."), ViewModel Display/ErrorMessages Vietnamese ("Mã công ty") — PaymentMethod validator + JournalEntry VM canonical.
- [journal-entry-handlers, G1] Loop chore (verifier stop-condition path change) committed separately as chore (precedent 6b5c3df) — never fold into task commit.
- [journal-entry-handlers, G2] `$"{x:D{width}}"` nested interpolation in a format specifier is invalid C# (CS1056 "Unexpected character '{'") — format strings cannot contain `{`; use `x.ToString($"D{width}")` for identical output (e.g. "JNRL000001").
- [journal-entry-handlers, G3] Nullable-struct `.Value` after a void method call → CS8629 under TreatWarningsAsErrors (compiler can't prove non-null; no [MemberNotNull] on void Post()): handler fix `entry.PostedAt!.Value` — null-forgiving justified because Post() throws (already-posted/unbalanced) BEFORE assigning PostedAt, so normal return ⇒ assigned; test fix `Assert.Equal(clock.Now, entry.PostedAt)` — implicit DateTimeOffset→DateTimeOffset? conversion, no dereference. RESEARCH claim "compiler allows .Value" was wrong — compiler is arbiter.
- [journal-entry-handlers, G3] IClock.Now is a property (`DateTimeOffset Now { get; }`), not a method — no parens; no FakeClock existed repo-wide before G3 (first handler to inject IClock, grep Application = 0 hits); add `internal sealed class FakeClock : IClock { public DateTimeOffset Now { get; set; } }` to Fakes.cs — settable Now, 5 lines.
- [journal-entry-handlers, G4] Evidence protocol: verifier re-runs every gate independently (build/arch/bank/migration-count/diff-tree sweep) — STATUS.md numbers are hypotheses, not facts; G4 evidence-only tasks make NO commit (evidence lives in STATUS.md, committed by next step).
- [journal-entry-handlers, G4] No-touch audit canon refinement (extends MEMORY:256): `git diff-tree --no-commit-id --name-only -r <commit>` beats `git show --stat` for machine-checking; include verifier state commits in the Migrations-touch sweep (they're loop commits too — state-files-only, but prove it, don't assume); `git log -- <file>` proves entity/port untouched (last-touched commit predates loop).
- [journal-entry-handlers, G2] Numbering-series atomicity: journal_entries.entry_number has NO unique index — the series row's xmin is the ONLY concurrency serialization point (concurrent creates → DbUpdateConcurrencyException on series row); single SaveChangesAsync persists entry + series.Increment() atomically; port has no UpdateAsync — change tracking persists the mutation.
- [journal-entry-handlers, G4] Wiring evidence = read the wiring files + build 0/0 as compiler confirmation: controller → MediatR.Send → internal handler (assembly-scanned) → port → single SaveChangesAsync → result record; all signatures verified by reading files, build proves they compile.
- [journal-entry-handlers, G4] Daily-screen write side complete: CreateJournalEntryHandler + PostJournalEntryHandler wire the Phiếu kế toán screen end-to-end (Index read side pre-loop); JournalEntry entity has NO CompanyId column — company scoping is command-level only, so future company-scoped JE features stay Application/Api-only (no migration).

### Inventory Module 03 — G1 Design Reconciliation (Sep 2026)
- [implement-module-inventory, G1] Partial-unique HasFilter on a soft-delete entity must include ALL mutable flags Deactivate() doesn't reset — `"is_primary"` alone on item_barcodes primary-unique recreates the deactivate→re-add dead-end (Deactivate() sets IsActive=false but never clears IsPrimary); canonical filter = `"is_primary" AND "is_active"` (extends MEMORY:263 single-flag lesson to multi-flag entities).
- [implement-module-inventory, G1] When transcribing a design deep-dive into a canonical record, a spec correction must be propagated to EVERY cross-referencing section (canonical block + tests list + risk-matrix row + migration shape) — the G1 canonical record dropped the handler null-guard during transcription; auditor caught it, verifier confirmed the fix landed in 3 places; verify canonical-vs-deep-dive sync before locking a design record.
