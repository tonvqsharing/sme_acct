...
### G3 BankTests candidate (NuGet — not referenced anywhere in repo, needs install)
- **Microsoft.EntityFrameworkCore.InMemory 10.0.4** (nuget.org) — optional EF round-trip for Bank handler tests; must match Microsoft.EntityFrameworkCore 10.0.4 in Infrastructure.csproj. Test-project-only; never add to Domain. Limitations: no unique-index/FK-Restrict/xmin enforcement. Prefer hand fakes (zero new packages) per RESEARCH.md "## Task-Specific Research — [G3] minimal integration tests".
