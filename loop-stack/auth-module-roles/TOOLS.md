...
(none — all findings below verified in-repo this session, not online)

## Session-Verified Test Inventory (for [G3] Tests executor)
- Canonical test csproj shape: `OutputType=Exe` + `UseAppHost=true` + `IsPackable=false` + `<Using Include="Xunit"/>` + `NoWarn $(NoWarn);CA1707` (Application.Tests + Web.Tests add `;xUnit1051`); Web.Tests uses `Sdk="Microsoft.NET.Sdk.Web"`. MTP runner via global.json — new test projects must copy shape or mix-fail exit 1.
- xunit v3 async pattern: `TestContext.Current.CancellationToken` passed to StartAsync/MigrateAsync/EF calls; containers via `IAsyncLifetime` (DatabaseIntegrationTests verbatim model).
- Testcontainers pin only `Testcontainers.PostgreSql 4.15.0` (+ XunitV3 4.15.0) + `Respawn 7.0.0`; Web.Tests already refs all three + Mvc.Testing, Security.Tests refs Mvc.Testing but NOT Testcontainers.
- BLOCKER: no `Microsoft.EntityFrameworkCore.Sqlite`/`SqlServer`/Pomelo pins in Directory.Packages.props (pre-existing NU1010) — per-provider SQLite smoke impossible without new CPM pin + lock regen. Fallback: `DbProviderSelector.Resolve` unit tests, no DB.
- Architecture `GetModuleAssembly` loads `artifacts/bin/{Project}/debug/{Project}.dll` — Release-only CI artifacts make it `return null` → rule silently skips; new arch tests must assert non-null.
- RolesController (MVC `Controller`, not ApiController): anonymous → 302 to `/Accounts/Login`; true 403 needs TestAuthHandler with `permission` claims. Failures map to 400, never 404.
