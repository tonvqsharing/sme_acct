...
### [G2] Consolidated Application+Infrastructure 2026-09-15
- [G2 Application] CQRS 6 sealed IRequest<Result> commands + 3 Result<IReadOnlyList<string>> queries as thin IRoleService/IPermissionService delegates with 6 FluentValidation validators (role regex ^[A-Za-z0-9_.-]+$, permission Must in Domain.Permissions.All, UserId NotEmpty); trap: ported PermissionPolicyProvider dead code unless registered, queries lack validators, no UpdateRole/GetRoleById, IPermissionService returns raw list/bool vs Result<T>.
- [G2 Infrastructure] RoleService/PermissionService thin over RoleManager<ApplicationRole>/UserManager<ApplicationUser> with lowercase "permission" claim const, idempotent assigns, IdentityFailure collapse, Ordinal distinct aggregation (unknown user empty not Fail); trade-offs: cross-module Infra→Identity.Infrastructure ref for concrete types (zero PackageReference), Guid userId vs long PK FindByIdAsync silent miss, dual static-19-policies + dynamic provider + Singleton-vs-Scoped handlers, Program.cs AddAuthorizationModule+UseAuthentication fix + mediator-only RolesController; infra+Api 0 new warnings, audit WARN verifier PASS.
### [G3] Seed wiring + migration verification 2026-09-15
- Program.cs startup (non-Testing env only): existing SmeAccountingDbContext pending-migration fail-fast kept, then IdentityDbContext.EnsureCreatedAsync + RoleSeeder.SeedAsync + UserSeeder.SeedAsync (ADMIN_PASSWORD env passthrough) + AuthorizationRoleSeeder.SeedAsync, all resolved from scoped DI with ILoggerFactory.CreateLogger("StartupSeed"); top-level await used, new usings Identity/Logging/Authorization.Seeding
- Seed order rationale: RoleSeeder creates 5 roles w/ permission claims (new roles only) → UserSeeder creates admin@smeaccounting.vn + AddToRoleAsync Admin (needs role to exist) → AuthorizationRoleSeeder reconciles claim deltas on existing roles (fixes RoleSeeder claim-drift gap) + re-assigns admin to Admin if missing
- Api Release build: 0 errors, only NU1608 Pomelo-9-vs-EF10 warnings (pre-existing, WarningsNotAsErrors); no csproj/lock changes needed (Api already refs both modules)
- has-pending-model-changes says model drifted; temp-migration probe proved drift is ONLY pre-existing multi-dbms fallout (xmin shadow drops on 4 tables + uuid-ossp annotation) — zero new tables, zero asp_net tables, zero Authorization tables; temp migration deleted + snapshot restored, Migrations/ clean
- Scope checks: no Migrations folder under Authorization/Infrastructure, 4 per-provider dirs (MariaDb/PostgreSql/Sqlite/SqlServer) all empty by design, Identity tables via EnsureCreated (IdentityDbContext still has zero migrations ever)
